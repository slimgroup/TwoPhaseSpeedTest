...
function TwoPhase(Kx::Array{T,3}, ϕ::Array{T,3}, qinj::Tuple{T1, T1, T1}, qrate::Number, d::Tuple{T2, T2, Vector{T2}}, time::Number, nt::Int; Ky=nothing, Kz=nothing, TOPS=nothing) where {T, T1, T2}
    "Kx permeability, ϕ porosity, qinj injection coordinate [m], qrate injection rate [Mt/y]"
...
function TwoPhase(Kx::Array{T,2}, ϕ::Array{T,2}, qinj::Tuple{T1, T1}, qrate::Number, d::Tuple{T2, T2, Vector{T2}}, time::Number, nt::Int; Ky=nothing, Kz=nothing, TOPS=nothing) where {T, T1, T2}
    n = size(Kx)
    if isnothing(Ky)
        Ky = reshape(Kx,n[1],1,n[2])
    else
...
function getqgrid(qinj::Tuple{T1, T1, T1}, d::Tuple{T3, T3, Vector{T3}}, TOPS::T) where {T, T1, T3}
    qgridxy = Int.(ceil.(qinj[1:2]./d[1:2]))
    depth = TOPS .+ cumsum(d[3])
    qgridz = findfirst(depth .>= qinj[3])
    return (qgridxy[1], qgridxy[2], qgridz)
end

function getqgrid(qinj::Tuple{T1, T1, T1}, d::Tuple{T3, T3, Vector{T3}}, TOPS::Vector{T}) where {T, T1, T3}
    qgridxy = Int.(ceil.(qinj[1:2]./d[1:2]))
    depth = TOPS[qgridxy[1]] .+ cumsum(d[3])
    qgridz = findfirst(depth .>= qinj[3])
    return (qgridxy[1], qgridxy[2], qgridz)
end

function getqgrid(qinj::Tuple{T1, T1, T1}, d::Tuple{T3, T3, Vector{T3}}, TOPS::Matrix{T}) where {T, T1, T3}
    qgridxy = Int.(ceil.(qinj[1:2]./d[1:2]))
    depth = TOPS[qgridxy[1], qgridxy[2]] .+ cumsum(d[3])
    qgridz = findfirst(depth .>= qinj[3])
    return (qgridxy[1], qgridxy[2], qgridz)
end
...
function WriteDZ(n::Tuple{Int, Int, Int}, d::Tuple{T, T, Vector{T}}) where T
    @assert length(d[3]) == n[3]
    dz = ["$(n[1]*n[2])*$(d[end][i])" for i = 1:n[3]]
    WriteTxtFile(dz; name="DZ")
    DZ = "INCLUDE
    'DZ.txt'
/"
    return DZ
...
function WriteDATAFile(qinj::Tuple{T1, T1, T1}, qrate::Number, n::Tuple{Int, Int, Int}, d::Tuple{T3, T3, Vector{T3}}, time::Number, nt::Int;
    TOPS=nothing, PERMXtxt::String = "PERMX.txt", PERMYtxt::String = "PERMY.txt", PERMZtxt::String = "PERMZ.txt", POROtxt::String = "PORO.txt", name="CO2SIMULATION") where {T1, T2, T3}
